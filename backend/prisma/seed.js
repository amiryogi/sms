...
const roles = [
  { name: 'SUPER_ADMIN', description: 'Super Administrator - Multi-school owner' },
...
const subjects = [
  ...
  { name: 'Physical Education', code: 'PE', description: 'Physical Education', isOptional: true },
...
async function main() {
  ...
  const adminUser = await prisma.user.upsert({
    where: { email_schoolId: { email: 'admin@demo-school.edu.np', schoolId: school.id } },
    update: {},
    create: {
      schoolId: school.id,
  ...
  console.log(`   ✓ Admin: admin@demo-school.edu.np / password123`);

  // Teacher User
  const teacherRole = await prisma.role.findUnique({ where: { name: 'TEACHER' } });
  const teacherUser = await prisma.user.upsert({
    where: { email_schoolId: { email: 'teacher@demo-school.edu.np', schoolId: school.id } },
    update: {},
    create: {
      schoolId: school.id,
  ...
  console.log(`   ✓ Teacher: teacher@demo-school.edu.np / password123`);
  ...
  const studentUser = await prisma.user.upsert({
    where: { email_schoolId: { email: 'student@demo-school.edu.np', schoolId: school.id } },
    update: {},
    create: {
      schoolId: school.id,
      email: 'student@demo-school.edu.np',
      passwordHash,
      firstName: 'Ram',
      lastName: 'Student',
  ...
  console.log(`   ✓ Student: student@demo-school.edu.np / password123`);

  // Parent User
  const parentRole = await prisma.role.findUnique({ where: { name: 'PARENT' } });
  const parentUser = await prisma.user.upsert({
    where: { email_schoolId: { email: 'parent@demo-school.edu.np', schoolId: school.id } },
    update: {},
    create: {
      schoolId: school.id,
      email: 'parent@demo-school.edu.np',
      passwordHash,
      firstName: 'Hari',
      lastName: 'Parent',
  ...
  console.log(`   ✓ Parent: parent@demo-school.edu.np / password123\n`);
