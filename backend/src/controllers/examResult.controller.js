...
const getTeacherExams = asyncHandler(async (req, res) => {
    ...
    const classSubjectIds = [...new Set(teacherSubjects.map((ts) => ts.classSubjectId))];
    ...
    const savedResults = await prisma.$transaction(async (tx) => {
        ...
        for (const result of validResults) {
            ...
            const upserted = await tx.examResult.upsert({
                ...
                update: {
                    marksObtained: result.isAbsent ? null : parseFloat(result.marksObtained) || null,
                    practicalMarks: result.practicalMarks ? parseFloat(result.practicalMarks) : null,
