...
const Assignments = () => {
  ...
  const openSubmissionModal = async (assignment) => {
    ...
    setSubmitting(true);
    try {
      const res = await assignmentService.getSubmissionsByAssignment(
        assignment.id
      );
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!selectedAssignment) {
        alert(
          "Invalid Class/Section selection. You are not assigned to this class."
        );
        setSubmitting(false);
        return;
      }
  ...
  const columns = [
    { header: "Title", accessor: "title" },
    {
      ...
      render: (row) =>
        `${row.teacherSubject?.classSubject?.class?.name || ""} - ${
          row.teacherSubject?.classSubject?.subject?.name || ""
        }`,
    },
    {
      header: "Section",
      render: (row) => row.teacherSubject?.section?.name || "-",
    },
    {
      header: "Due Date",
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <FormRow>
            ...
            <Select
              ...
              name="sectionId"
              options={[
                ...new Set(teacherAssignments.map((ta) => ta.sectionId)),
              ].map((id) => {
                const ta = teacherAssignments.find((t) => t.sectionId === id);
                return {
                  value: id?.toString(),
                  label: ta?.section?.name || "Section",
                };
              })}
              register={register}
              required
          ...
          <div className="form-group">
            ...
            <div className="file-upload-wrapper">
              ...
              {editingAssignment &&
                editingAssignment.assignmentFiles?.length > 0 && (
                  <div className="mt-2">
                    ...
                    <div className="file-list">
                      {editingAssignment.assignmentFiles.map((f, i) => (
                        <a
                          ...
                          href={f.fileUrl}
                          target="_blank"
                          className="file-item text-primary"
          ...
          <div className="modal-actions">
            ...
            <Button
              type="submit"
              loading={submitting}
              icon={editingAssignment ? null : Upload}
            >
                      ...
                      <td>
                        <div className="user-cell">
                          {sub.student.user.avatarUrl ? (
                            <img
                              src={resolveAssetUrl(sub.student.user.avatarUrl)}
                              alt=""
                              className="user-avatar-sm"
                            />
                          ) : (
                            <div className="user-avatar-placeholder-sm">
                              {sub.student.user.firstName[0]}
                            </div>
                          )}
                          <span>
                        ...
                        {grading === sub.id ? (
                          <div className="d-flex gap-2">
                            <input
                              ...
                              placeholder="Marks"
                              style={{ width: "80px" }}
                              {...gradingForm.register("marksObtained", {
                                required: true,
                              })}
                            />
                          </div>
                      ...
                      <td>
                        {grading === sub.id ? (
                          <div className="d-flex gap-2">
                            <Button
                              size="sm"
                              onClick={gradingForm.handleSubmit((data) =>
                                onGradeSubmit(data, sub.id)
                              )}
                            >
                              Save
                            </Button>
